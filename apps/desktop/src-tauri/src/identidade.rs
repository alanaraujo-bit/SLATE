...
pub enum ErroIdentidade {
    #[error("não foi possível acessar a pasta de dados do aplicativo")]
    PastaIndisponivel,
...
mod testes {
    ...
    fn a_mesma_identidade_persiste_entre_execucoes() {
        ...
        let primeira = Identidade::carregar_ou_criar(&pasta).unwrap().chave_publica();
        let segunda = Identidade::carregar_ou_criar(&pasta).unwrap().chave_publica();
    ...
    fn a_chave_privada_nao_fica_em_texto_claro_no_disco() {
        ...
        if cfg!(windows) {
            ...
            assert!(
                !bruto.windows(publica.len()).any(|j| j == publica.as_slice()),
